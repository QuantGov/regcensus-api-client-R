...
There are six helper functions to retrieve information about these key components of regdata. These functions provider the following information: topics, documents, jurisdictions, series, agencies, and years with data. The list functions begin with __list__. For example, to view the list of topics call __list_topics()__. When an topic id parameter is supplied, the function returns the details about a specific topic. 
```{r}
#list_topics()
```

Each topic comprises one or more *series*. The __list_series()__ function returns the list of all series when no series id is provided. 
...
```
The __list_seriesyear()__ function returns a list of all seriesa nd the years with data available. 
